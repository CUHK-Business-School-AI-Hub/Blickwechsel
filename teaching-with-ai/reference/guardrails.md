<!-- Generated from src/ai-teaching-guardrails/SKILL.md - do not edit this file.
...
Apply these whenever the person will put their name on the output. Every other part of this skill assumes these rules; this file states them, and stands on its own for anything else.
...
**8. Never cite what has not been opened.** Invented and misattributed references are the characteristic failure of these tools in academic work, and they cost more socially than a wrong number. Open the source, or do not cite it. The `reference/verify.md` skill is the mechanism for this: it checks every citation and number with a verifier that never sees the draft.
